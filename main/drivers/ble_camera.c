...
esp_err_t ble_camera_connect(const char* device_name) {
    ...
    ESP_LOGI(TAG, "Starting scan for camera device: %s", device_name);
    ...
    ret = esp_ble_gap_start_scanning(10); // Scan for 10 seconds
    if (ret) {
...
esp_err_t ble_camera_disconnect(void) {
    ...
    if (camera_status == BLE_CAMERA_STATUS_STREAMING) {
        send_camera_command(0x02, NULL, 0); // STOP_STREAM command
    }
    
    // Close GATT connection if connected
    ...
    switch (event) {
        ...
        case ESP_GATTC_OPEN_EVT:
            if (param->open.status == ESP_GATT_OK) {
                ESP_LOGI(TAG, "Connected to camera device");
        ...
        case ESP_GATTC_SEARCH_RES_EVT:
            ESP_LOGI(TAG, "Service found: UUID16 0x%04x", param->search_res.srvc_id.uuid.uuid.uuid16);
            break;
    ...
    switch (event) {
        case ESP_GAP_BLE_SCAN_RESULT_EVT: {
            ...
            if (scan_result->scan_rst.search_evt == ESP_GAP_SEARCH_INQ_RES_EVT) {
                // Check if this is our target camera device
                ...
                if (adv_name != NULL && adv_name_len > 0) {
                    ...
                    if (strcmp(device_name, target_device_name) == 0) {
                        ESP_LOGI(TAG, "Target camera found, stopping scan...");
                        esp_ble_gap_stop_scanning();
                        
                        // Attempt to connect
                        esp_ble_gattc_open(gattc_if, scan_result->scan_rst.bda, 
